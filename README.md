...
- Overrides Anki's contenteditable default spacing issue by removing non-breaking spaces `&nbsp;` exclusively from MathJax tags to prevent rendering breakage.
...
- `remove_nbsp_in_mathjax`: boolean to enable scrubbing non-breaking spaces `&nbsp;` automatically generated by Anki's editor (e.g., from typing double spaces or pasting via web browser). This clears them exclusively inside MathJax blocks to avoid LaTeX compilation failures.
- Default: `true`
...
- Added automatic removal of non-breaking spaces `&nbsp;` from inside native MathJax limits (`\(`) and (`\[`) and Anki tags to prevent rendering errors caused by Anki's rich-text editor defaults, configurable via `remove_nbsp_in_mathjax` option and available natively in the settings UI.
